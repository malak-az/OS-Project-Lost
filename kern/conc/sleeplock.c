...
void acquire_sleeplock(struct sleeplock *lk)
{
...
#if USE_KHEAP
	//cprintf("sleep lock\n");
    struct kspinlock* guard = &(lk->lk);


    //if lk held >> sleep() / else hold the lk
    ...
    lk->locked = 1;
    //lk->pid = get_cpu_proc()->env_id; //check in holding
...
void release_sleeplock(struct sleeplock *lk)
{
    ...
    //Your code is here
#if USE_KHEAP
	//cprintf("release spin\n");
    struct kspinlock* guard = &(lk->lk);

    acquire_kspinlock(guard);

    //if anyone in wait qu >> wakeup all // release
    struct Channel* chan = &(lk->chan);
    wakeup_all(chan);
