...
struct Env* fos_scheduler_PRIRR()
{
	...
	//1.remove el curr
	if(current_env!=NULL){
		//int pri=current_env->priority;
		//current_env->env_status=ENV_READY;
		//enqueue(&(ProcessQueues.env_ready_queues[pri]), current_env);
		 sched_insert_ready(current_env);
		 current_env->clockint=0;
	}
	...
	for(int i=0;i<num_of_ready_queues;i++){
		...
		if (size!=0){
			...
			//nxt_env->env_status==ENV_UNKNOWN;
			kclock_set_quantum(quantums[0]);
            return nxt_env;
		}
	}
		...
		acquire_kspinlock(&ProcessQueues.qlock);
	        ...
	        for(int i=1;i<num_of_ready_queues;i++){
	            ...
	            if (size!=0){
	               ...
	               LIST_FOREACH_SAFE(proc,&(ProcessQueues.env_ready_queues[i]),Env){

	                    if (proc->clockint >= Global_StarvThresh) {
