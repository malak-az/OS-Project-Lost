...
//    panic("allocate_user_mem() is not implemented yet...!!");
    ...
    int ret= get_page_table(e->env_page_directory,  virtual_address+i*PAGE_SIZE, &Page_table);
     ...
     pt_set_page_permissions(e->env_page_directory, virtual_address+i*PAGE_SIZE, PERM_UHPAGE, 0);}
...
//	cprintf("free user mem\n");
    ...
    for(int i=0;i<pages;i++){
        LIST_FOREACH(w,&(e->page_WS_list)){
            if(w->virtual_address==virtual_address+i*PAGE_SIZE)

             env_page_ws_invalidate( e,  virtual_address+i*PAGE_SIZE);

        } uint32 perm=PERM_UHPAGE|PERM_PRESENT|PERM_USER|PERM_AVAILABLE;

         pf_remove_env_page(e, virtual_address+i*PAGE_SIZE);

         pt_set_page_permissions(e->env_page_directory, virtual_address+i*PAGE_SIZE, 0, perm);
