...
void env_free(struct Env *e)
{
	...
	LIST_FOREACH_SAFE(w,&e->page_WS_list,WorkingSetElement){
		my_va=w->virtual_address;
		cprintf("list");
		env_page_ws_invalidate(e,w->virtual_address);
		if (!LIST_EMPTY(&AllShares.shares_list)){
			...
			LIST_FOREACH(sh, &AllShares.shares_list){
				...
				int ret=delete_shared_object(sh->ID,(void *)w->virtual_address);
				if (!holdd)
	...
	for (int i=0;i<PDX(USER_TOP);i++)
	{
		if (!(e->env_page_directory[i] & PERM_PRESENT))
		    continue;
		uint32 add = EXTRACT_ADDRESS(e->env_page_directory[i]);
		uint32 *pt = (uint32 *)kheap_virtual_address(add);
		uint32 va;
        kfree(pt);
	}
	kfree((void*)e->prepagedVAs);
	kfree((void*)e->kstack);
...
#if USE_KHEAP
    uint32 *v_add=kmalloc(KERNEL_STACK_SIZE);
    bool valid=(v_add!=NULL);
    //kmalloc ya nadaaa
    if(valid==0 || ptr_user_page_directory==NULL)
        panic("for user kernel stack: it cannot be created");
    else{
        uint32 *pg_table=NULL;
        uint32 vir_add=(uint32)v_add;
        //int get_page_table(uint32 *ptr_page_directory, const uint32 virtual_address, uint32 **ptr_page_table)
        get_page_table(ptr_user_page_directory,vir_add,&pg_table);
        //guard page is still missing do not forget Basmala
        pg_table[PTX(v_add)] &=(~PERM_PRESENT);
    }
    return v_add;
#else
    uint32 rem_spc=KERNEL_HEAP_MAX-__cur_k_stk;
    if(rem_spc<KERNEL_STACK_SIZE)
        panic("for user kernel stack: cannot allocate kernel stack");
    void* ker_stk = (void*)__cur_k_stk;
